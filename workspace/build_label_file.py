...
if __name__== "__main__":
    ...
    src_part_paths = sorted(glob.glob(f"datasets/{sel_ds_id}_parts/labels_src*"), key=lambda p: int(p.split('_')[-1]))
    simp_part_paths = sorted(glob.glob(f"datasets/{sel_ds_id}_parts/labels_simp*"), key=lambda p: int(p.split('_')[-1]))
    ...
    pickle.dump(src_labels, open(f"{ds_label_path}/{sel_ds_id}_src_labels.pkl", "wb"))
    pickle.dump(simp_labels, open(f"{ds_label_path}/{sel_ds_id}_simp_labels.pkl", "wb"))
